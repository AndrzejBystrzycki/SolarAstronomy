...
public final class SPA {
    ...
    public static AzimuthZenithAngle calculateSolarPosition(final GregorianCalendar date, final double latitude,
        ...
        Assert.msg("julianDate="+jd.getJulianDate());
        Assert.msg("julianEphemerisDay="+jd.getJulianEphemerisDay());
        Assert.msg("julianJulianCentury="+jd.getJulianCentury());
        ...
        Assert.msg("lDegrees="+lDegrees);

        // calculate Earth heliocentric latitude, B
        final double[] bTerms = calculateLBRTerms(jme, TERMS_B);
        // A diference from "Solar Position Algorithm for Solar Radiation Applications" 
        // by Ibrahim Reda and Afshin Andreas
        // The algorithm by Reda and Andreas does not limit bDegrees to 360
        ...
        Assert.msg("thetaDegrees="+thetaDegrees);
        ...
        Assert.msg("deltaTau="+deltaTau);
        Assert.msg("deltaTau="+TimeFormatter.formatHoursHHMMSS(deltaTau));
        
        // calculate the apparent sun longitude
        final double lambdaDegrees = thetaDegrees + deltaPsi + deltaTau;
        Assert.msg("lambdaDegrees="+lambdaDegrees);
        Assert.msg("lambdaDegrees="+TimeFormatter.formatHoursHHMMSS(lambdaDegrees));
        ...
        // test
        // The data from Astronomical Algorithms.pdf
        /*double a = TimeFormatter.convertHMStoDH(199, 54, 21.818); // lambda page 177
        double b = Math.toRadians(a);
        double c = TimeFormatter.convertHMStoDH(0, 0, 0.62); // beta page 177
        double d = Math.toRadians(c);
        double e = Math.toRadians(23.4401443); // epsilon page 177
        double alphaTest = calculateGeocentricSunRightAscension(d, e, b);
        System.out.println("alphaTest deg="+alphaTest);*/
        ...
        Assert.msg("deltaDegrees="+deltaDegrees);
        ...
        final double EOT = calculateEuqationOfTime(jme, alphaDegrees, deltaPsi, epsilon);
        Assert.msg("EOT="+EOT);
     ...
     */
    public static double calculateEuqationOfTime(double jme, double rightAscention, double deltaPsi, double epsilon)
    {
    	Assert.msg("jme="+jme);
    	
    	double M = 280.4664567 + jme*(360007.6982779 + jme*(0.03032028 +
                jme*(1/49931.0   + jme*(-1/15300.0     + jme*(-1/2000000.0)))));
    	Assert.msg("M="+M);
    	M = limitDegreesTo360(M);
    	Assert.msg("M limit="+M);
    	
    	double EOTDeg = M - 0.0057183 - rightAscention + deltaPsi * Math.cos(epsilon);

    	Assert.msg("rightAscention="+rightAscention);
    	Assert.msg("rightAscention hrs="+rightAscention/15);
    	Assert.msg("deltaPsi="+deltaPsi);
    	Assert.msg("epsilon="+epsilon);
    	Assert.msg("epsilon deg="+Math.toDegrees(epsilon));
    	Assert.msg("EOTDeg="+EOTDeg);
    	
    	double EOTMin = EOTDeg * 4;
    	
    	EOTMin = limit_minutes(EOTMin);
    ...
    private static double limit_minutes(double minutes)
    {
